...
function memoryLoss(memory::KNNmemory{T}, normalizedQuery::AbstractArray{T, 1}, nearestPositiveID::Integer, nearestNegativeID::Integer) where {T}
    loss = max(dot(normalizedQuery, memory.encoder(memory.M[nearestNegativeID, :])) - dot(normalizedQuery, memory.encoder(memory.M[nearestPositiveID, :])) + memory.α, 0)
end

...
const dataFolder = "D:/dev/julia/OSL/experiments/WSVAElargeSVAEanom/"
...
params = [:hidden, :latent, :layers, :nonlinearity, :layertype, :β, :method, :anomaliesSeen, :auc, :ar, :i, :model, :dataset]
# types = [Int, Int, Int, String, String, Union{Int, Missings.Missing}, Union{Int, Missings.Missing}, Int, Float64, Float64, String, String]
types = [Int, Int, Int, String, String, Float64, String, Int, Float64, Float64, Int, String, String]
    ...
    for i in 1:10
        ...
        for i in 1:length(results)
            for j in 1:length(results[1][2])
                ...
                push!(dataframe, vcat(pars..., results[i][2][j][1:3]..., results[i][2][j][5:6]..., model, dataset))
            end
        end
    end
end

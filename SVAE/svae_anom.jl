...
mutable struct SVAE_anom
	...
	"""
	SVAE_anom(q, g, hdim::Integer, zdim::Integer, T = Float32) = new(q, g, zdim, convert(T, huentropy(zdim)), Adapt.adapt(T, Chain(Dense(hdim, zdim), x -> normalizecolumns(x))), Adapt.adapt(T, Dense(hdim, 1, softplus)), Flux.param(Adapt.adapt(T, normalize(randn(zdim)))), Flux.param(Adapt.adapt(T, [1.])))
end
...
function wloss_anom_lkh(m::SVAE_anom, x, y, β, d)
	...
	Ω = d(z, zp)
	Ωanom = d(z, anom_prior)
	xgivenz = m.g(z)
	return (1 .- y) .* (Flux.mse(x, xgivenz) + β * Ω) .+ y .* (-log_vmf(z, m.anom_priorμ, m.anom_priorκ))
end

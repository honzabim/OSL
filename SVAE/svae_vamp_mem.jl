...
function var_posterior_lkh(m::SVAE_vamp, z, pseudo_inputs)
	(μz, κz) = zparams(m, pseudo_inputs)
	l = 0
	for K in 1:size(pseudo_inputs, 2)
		l += exp.(log_vmf(z, μz[:, K], κz[K]))
	end
	l /= size(pseudo_inputs, 2)
	return l
end
